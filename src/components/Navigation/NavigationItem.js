                ...
                <div className="relative grid gap-1 p-2 px-2 lg:grid-cols-1 bg-white w-auto rounded-xl font-customFont ">
                  {children?.map(({ name, href }) => (
                    <Link
                      ...
                      className={`${
                        pathname === href
                          ? 'bg-secondary text-primary font-customFont flex items-center  rounded-lg transition duration-150 ease-in-out hover:bg-secondary focus:outline-none focus-visible:ring'
                          : 'flex items-center rounded-lg transition duration-150 ease-in-out  hover:bg-secondary-opacity focus:outline-none focus-visible:ring'
                      }`}
                      onClick={async () => {close()}}
                    >
                      <div className="ml-3">
                        <p className="text-sm py-2 font-normal font-customFont ">{name}</p>
                      </div>
                    </Link>
                  ))}
                </div>
